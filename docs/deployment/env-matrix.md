...
| `RESEND_FROM_EMAIL`    | apps/web | P, V, D      | Set manually          | **Phase 1/demo**: `onboarding@resend.dev` (Resend sandbox). Sandbox emails deliver ONLY to the Resend account owner email (`archer@1stimpression.co`). Adequate for Phase 1 demo where Archer is the demonstrator. **Phase 4**: switch to `no-reply@alphawolfwrap.com` after domain verification at the registrar and DNS propagation. Phase 4 issue logged. | Update Vercel + `.env.local`; set `AUTH_EMAIL_TRANSPORT=` (unset) to re-enable live delivery. | Archer |
| `AUTH_EMAIL_TRANSPORT` | apps/web | D (optional) | Set to `console`      | Bypass Resend in local dev; OTP prints to terminal AND ring buffer. Never set in production.                                                                                                                                                                                                                                                                 | Remove from env to restore live email.                                                        | Archer |
...
| Env var                        | Notes                                                          |
| ------------------------------ | -------------------------------------------------------------- |
| `AUTH_EMAIL_TRANSPORT=console` | Skip Resend; print OTP to stdout.                              |
| `AUTH_EMAIL_TRANSPORT` (unset) | Live Resend delivery (sandbox sender `onboarding@resend.dev`). |
