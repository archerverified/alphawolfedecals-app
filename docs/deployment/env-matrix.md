...
| `ANTHROPIC_ORCHESTRATOR_MODEL` | apps/web                                  | P, V, D | n/a (config value)               | Orchestrator model (brief to per-view prompts). Optional. Unset = `claude-sonnet-4-6` (the PRD default). Allowlist: `claude-sonnet-4-6`, `claude-opus-4-8`, `claude-haiku-4-5`. Any other value fails fast at run time. Text only (cents/run); fal dominates run cost. | Change the value (no secret); redeploy.                               | Archer |
