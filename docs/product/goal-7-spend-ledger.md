...
| 4 | 2026-06-12 11:1x–12:2x | fal + Anthropic | REAL PROD PROOF RUN (brief → 3 concepts → 1 iteration → final → export pack; X3, real fal renders; from generation_runs cost rows before cleanup) | $0.6994 fal + ~$0.02 Haiku | $1.09 | $0.09 |

**FINAL TOTALS: fal ≈ $1.09 / $6.00 · Anthropic ≈ $0.09 / $3.00 — combined ≈ $1.18 of the $9.00 ceiling.** (Server-side bake-off ledger in project-assets `bakeoff/2026-06/ledger.json` is the per-call source of truth for bake-off fal calls; proof-run costs were read from generation_runs cost rows before artifact cleanup.)
