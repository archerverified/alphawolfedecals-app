// Goal 16 — full-journey B2C acceptance E2E on the BMW X3 (AW-TPL-0001).
//
// Adapted verbatim from goal-13-full-journey.spec.ts (same POM/selectors). A
// single durable test that drives the COMPLETE customer journey end-to-end and
...
//
// LOCAL DEV ONLY. It uses the dev-otp peek (404 in production). On MOCK it is
// free (AI_PROVIDER=mock); the headline Goal 16 proof run flips
// AI_PROVIDER=fal in .env.local for ONE real-fal run (real Haiku orchestrator +
// real fal images). It green-skips on any remote DEPLOY_URL target — same guard
// shape as goal-13. It must never join the prod smoke.
//
// LOCKED BRIEF (Archer, Goal 16):
//   - 2024 BMW X3.
//   - Design = gloss black → cyan GRADIENT — a bold cohesive gradient across ALL
//     panels, gloss finish (NOT flat). Style prompt states exactly that.
//   - Alpha Wolf logo composited on the driver-side door AND passenger-side door
//     (both — the must-have) PLUS the hood (a hood zone exists on the X3).
//   - Full wrap (every panel) so the gradient is cohesive end-to-end.
//
...
// calibrated, selectable wrap zones). Same id as goal-12/goal-13.
...
// rendered button label so we don't hard-code panel UUIDs (logo-zone-<panelId>).
const LOGO_ZONE_NAMES = ['Front Door', 'Hood Front'] as const;
...
class BriefWizardDriver {
  ...
  /**
   * Step 1 — zones: KEEP the full wrap (every panel) so the gloss-black→cyan
   * gradient is cohesive end-to-end (the locked brief). Assert the full-wrap
   * default holds and all logo zones stay assignable.
   */
  async zones(): Promise<void> {
    ...
    // Default is full wrap — every panel included. Don't toggle anything out;
    // the gradient needs the whole vehicle, and all 3 logo zones (both front
    // doors + hood) must remain assignable in the logo step.
    await expect(this.page.getByTestId('zone-summary')).toHaveText(/full wrap/i);
   ...
   * then assign it to BOTH front doors AND the hood (the Goal 16 must-have).
   */
  async logo(): Promise<void> {
    ...
    // Assign the logo to BOTH front doors (driver + passenger) AND the hood.
    // logo-zone buttons render the panel name as their text — both front doors
    // are labeled "Front Door", the hood is "Hood Front". Click every matching
    // chip and assert it became pressed.
    const logoZoneButtons = this.page.locator('[data-testid^="logo-zone-"]');
    let assigned = 0;
    const count = await logoZoneButtons.count();
    for (let i = 0; i < count; i++) {
      const btn = logoZoneButtons.nth(i);
      const label = ((await btn.textContent()) ?? '').trim();
      if ((LOGO_ZONE_NAMES as readonly string[]).includes(label)) {
        await btn.click();
        await expect(btn).toHaveAttribute('aria-pressed', 'true');
        assigned += 1;
      }
    }
    // The X3 has driver Front Door + passenger Front Door + Hood Front = 3.
    // Both doors are the hard must-have; the hood is included when present.
    expect(assigned).toBeGreaterThanOrEqual(2);
    // Sanity: at least the two doors landed (and the hood, which exists here).
    const pressed = await this.page
      .locator('[data-testid^="logo-zone-"][aria-pressed="true"]')
      .count();
    expect(pressed).toBe(assigned);
    await shot(this.page, '10-brief-logo');
  }

  /**
   * Step 4 — colors: land the gradient endpoint colors (gloss black + cyan) in
   * color-picks via the native picker (deterministic; extract-from-logo is
   * best-effort below).
   */
  ...
  /** Step 5 — style: the Aggressive preset + the gloss-black→cyan gradient prompt. */
  ...
  async aiNotes(): Promise<void> {
    ...
    await notes.fill(
      'Alpha Wolf is a marine / off-road brand. The whole vehicle is ONE continuous ' +
        'gloss-black-to-cyan gradient — keep it cohesive and glossy across every panel.',
    );
...
class EditorDriver {
  ...
  async exportSpecPack(): Promise<void> {
    ...
    fs.mkdirSync(SHOT_DIR, { recursive: true });
    fs.writeFileSync(path.join(SHOT_DIR, 'goal-16-export-pack.pdf'), pdf);
...
test.describe('Goal 16 — full B2C customer journey on the BMW X3 (gloss-black→cyan gradient)', () => {
  test.skip(
    isRemoteTarget(),
    'Full-journey acceptance — local dev only (dev-otp). Production uses the real provider.',
  );
  ...
  test('landing → signup → catalogue → brief → generate → editor → export', async ({
    ...
    // teardown all share one budget. On REAL fal each generation stage runs
    // 30–120s, so keep the wide ceiling.
    test.setTimeout(900_000);

    // @e2e.alphawolf.test suffix (the retire cohort) is baked into uniqueEmail.
    const email = uniqueEmail('g16');
