#!/usr/bin/env python3
"""
Idempotent GitHub issue seeder for Alpha Wolf Wrap Studio.

- Lists existing issues via `gh issue list`.
- Skips any whose title starts with `[GH-XXX]` already present.
- Creates only the missing ones with the right labels.

Safe to re-run. Requires `gh` CLI authenticated.

Usage:
    python3 scripts/seed-issues.py
"""
...
ISSUES = [
    {
        "id": "GH-001",
        "title": "Customer signup with email verification",
        "labels": ["phase-1", "auth"],
        "body": """**Description**
...
""",
    ...
    {
        "id": "GH-002",
        "title": "Shop signup with org creation",
        "labels": ["phase-1", "auth"],
        "body": """**Description**
...
""",
    ...
    {
        "id": "GH-003",
        "title": "Vehicle template browse and select",
        "labels": ["phase-1", "vehicle-db"],
        "body": """**Description**
...
**Acceptance criteria**
- [ ] Cascade selector: Year (1990 to current year) then Make then Model then Trim. Each level loads in under 200ms on cached data.
- [ ] Free-text search box matches across all fields with typo tolerance (transt 250 matches Transit 250).
...
""",
    ...
    {
        "id": "GH-004",
        "title": "Internal admin vehicle template CRUD",
        "labels": ["phase-1", "vehicle-db"],
        "body": """**Description**
...
- [ ] SVG upload validated for 4 views (front/back/driver/passenger), wrap-safe zones as named paths, no embedded raster over 500KB.
...
""",
    ...
    {
        "id": "GH-005",
        "title": "Asset upload with vector parsing",
        "labels": ["phase-1", "editor"],
        "body": """**Description**
As a user, I want to upload my logo in AI/EPS/PDF/SVG/PNG/JPG and have it parsed into a usable asset so that I do not have to convert formats manually.

**Acceptance criteria**
- [ ] Accepts AI, EPS, PDF, SVG, PNG, JPG, HEIC. Rejects others with clear message.
- [ ] Max file size 50MB; client + server enforcement.
- [ ] Vector formats parsed server-side via Inkscape; preview PNG returned within 10 seconds for files under 10MB.
...
**Dependencies:** None
**Phase:** 1
**Source PRD:** Section 10.5
""",
    },
    {
        "id": "GH-006",
        "title": "Initial AI design generation",
        "labels": ["phase-2", "ai"],
        "body": """**Description**
...
**Acceptance criteria**
- [ ] Requires selected vehicle template, at least 1 brand asset, style preset chip.
- [ ] Optional free-text prompt up to 500 chars.
- [ ] Submit triggers background job; UI shows step-labelled progress.
- [ ] p95 end-to-end under 90s; hard timeout at 180s with retry prompt.
...
- [ ] PDFs carry provenance signature (Generated with Alpha Wolf Wrap Studio with model name and timestamp).
...
""",
    ...
    {
        "id": "GH-007",
        "title": "Natural-language design tweaks",
        "labels": ["phase-2", "ai"],
        "body": """**Description**
...
**Dependencies:** GH-006
**Phase:** 2
**Source PRD:** Section 10.7
""",
    },
    {
        "id": "GH-008",
        "title": "Canvas editor with per-panel masking",
        "labels": ["phase-1", "editor"],
        "body": """**Description**
...
**Source PRD:** Section 10.8
""",
    },
    {
        "id": "GH-009",
        "title": "Shop printer and media setup",
        "labels": ["phase-3", "print-paneling"],
        "body": """**Description**
...
- [ ] Fields: printer model (preset list + Other), media width (in, 0.25 inch steps), laminate width, default overlap (default 0.5 inch), panel direction, default bleed (default 0.25 inch).
- [ ] Multiple printer profiles per shop; one default.
- [ ] Settings version-stamped on each export for reproducibility.

**Dependencies:** GH-002
**Phase:** 3
**Source PRD:** Section 10.9
""",
    },
    {
        "id": "GH-010",
        "title": "Automatic print paneling",
        "labels": ["phase-3", "print-paneling", "export"],
        "body": """**Description**
...
**Acceptance criteria**
- [ ] Triggered from project then Export then Production Package.
...
- [ ] Aligns panel seams to body panel breaks where geometry is within 2 inches of a media-width interval.
...
- [ ] Validation pre-flight blocks export on bleed underflow, panel exceeds media width, missing wrap-safe data, raster under 150 DPI at final size.
- [ ] Generates in under 30s (p95) for a full vehicle wrap.

**Dependencies:** GH-008, GH-009
**Phase:** 3
**Source PRD:** Section 10.10
""",
    },
    {
        "id": "GH-011",
        "title": "Detailed export with full metadata",
        "labels": ["phase-3", "export"],
        "body": """**Description**
...
**Acceptance criteria**
- [ ] Cover sheet contains four labelled sections: Vehicle, Design, Print Production, Project Tracking (per PRD Section 4.8).
...
- [ ] Wrap coverage percent calculated from total wrap surface area divided by total vehicle surface area (whole percent).
- [ ] Project Tracking section reflects current project state (status, version, dates, names).
- [ ] AI provenance signature in PDF footer.

**Dependencies:** GH-010
**Phase:** 3
**Source PRD:** Section 10.11
""",
    },
    {
        "id": "GH-012",
        "title": "Project handoff via token",
        "labels": ["phase-4", "ux"],
        "body": """**Description**
...
**Acceptance criteria**
- [ ] Send to a shop flow lets user pick Alpha Wolf Decals (default) or enter a 12-char shop code.
- [ ] Generates single-use token tied to project; emails shop admin.
- [ ] Shop accepts and project is mirrored into shop workspace with full version history + assets; shop becomes editor.
...
**Phase:** 4
**Source PRD:** Section 10.12
""",
    },
    {
        "id": "GH-013",
        "title": "Project activities log",
        "labels": ["phase-3", "activity-log"],
        "body": """**Description**
...
- [ ] Exported markdown matches the same schema as the repo-level `activities.md` (PRD Section 11.7).
...
""",
    ...
    {
        "id": "GH-014",
        "title": "Material and labor estimator on export",
        "labels": ["phase-5", "export"],
        "body": """**Description**
...
""",
    },
    {
        ...
        "body": """**Description**
As a user, I want to receive email and in-app notifications for actions that need my attention so that I do not have to keep checking the app.
...
- [ ] Email templates score over 9/10 on Mail Tester, include unsubscribe link, pass SPF/DKIM/DMARC.

**Dependencies:** GH-001
**Phase:** 4
**Source PRD:** Section 10.16
""",
    },
    {
        "id": "GH-017",
        "title": "Vehicle template request loop",
        "labels": ["phase-4", "vehicle-db"],
        "body": """**Description**
As a user, I want to request a vehicle that is not in the database and receive an email when the template ships so that I can come back and design.
...
**Phase:** 4
**Source PRD:** Section 10.17
""",
    },
    {
        "id": "GH-018",
        "title": "Generation cost dashboard for shops",
        "labels": ["phase-4", "ai"],
        "body": """**Description**
...
**Acceptance criteria**
- [ ] Settings then Delete Account with double-confirmation and password re-entry.
...
**Dependencies:** GH-001
**Phase:** 4
**Source PRD:** Section 10.19
""",
    },
    {
        "id": "GH-020",
        "title": "Security: auth and session hardening",
        "labels": ["phase-4", "security", "auth"],
        "body": """**Description**
...
- [ ] Rate-limit: 5 failed logins per IP per 15 min then lockout with backoff; per-account lockout after 10 failures.
...
**Phase:** 4
**Source PRD:** Section 10.20
""",
    },
    {
        "id": "GH-021",
        "title": "Comment threads on projects and versions",
        "labels": ["phase-4", "ux"],
        "body": """**Description**
...
- [ ] Comments immutable once posted; editing creates an edited badge with diff on hover.
...
**Phase:** 4
**Source PRD:** Section 10.21
""",
    },
    {
        "id": "GH-022",
        "title": "Asset upload IP acknowledgement",
        "labels": ["phase-4", "security"],
        "body": """**Description**
...
**Dependencies:** GH-005
**Phase:** 4
**Source PRD:** Section 10.22
""",
    },
]

# All label names referenced above
ALL_LABELS = {
    "phase-1", "phase-2", "phase-3", "phase-4", "phase-5", "phase-6",
    "auth", "infra", "ai", "editor", "vehicle-db", "export",
    "print-paneling", "security", "ux", "activity-log",
}
...
def existing_issue_ids() -> set[str]:
    """Return the set of GH-XXX IDs already on the repo."""
    result = run([
        "gh", "issue", "list",
        "--repo", REPO,
        "--state", "all",
...
def create_issue(issue: dict) -> Optional[str]:
    """Create one issue. Returns the issue URL on success, None on failure."""
    cmd = [
        "gh", "issue", "create",
        "--repo", REPO,
    ...
    try:
        ...
        print(f"  ERROR creating {issue['id']}: {e.stderr}")
        return None
...
def main() -> int:
    print(f"Seeding issues on {REPO}\n")

    # Sanity check that gh is authed
    try:
    ...
    print(f"Found {len(existing)} existing GH-XXX issues. Skipping these.\n")
    ...
    for issue in ISSUES:
        if issue["id"] in existing:
            ...
            skipped += 1
            continue
        print(f"CREATE {issue['id']}: {issue['title']}")
